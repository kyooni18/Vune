...
function template(source) {
  return readFileSync(resolve(packageRoot, source), 'utf8')
    .replaceAll('__MUSE_VERSION__', packageManifest.version)
}
...
function writeTemplates(projectRoot, files) {
  for (const [source, destination] of files) {
    ...
    writeFileSync(target, template(source).replaceAll('__MUSE_PROJECT_NAME__', projectName(projectRoot)))
  }
}
...
Usage:
  muse create <directory> [--pm <manager>] [--no-install] [--force]
  muse init [--force] [--no-install]

create scaffolds a ready-to-run canonical Muse app with Vite, React, and
...
function scaffold(projectRoot, commandName) {
  if (existsSync(projectRoot) && !options.force && readdirSync(projectRoot).length > 0) {
    console.error(`Muse ${commandName} cannot use a non-empty directory: ${projectRoot}`)
  ...
  console.log(`Created canonical Muse app in ${projectRoot}`)
...
function main() {
  ...
  if (command === 'create' || command === 'new') {
    ...
    if (!target || positionals.length > 1) {
      console.error('Usage: muse create <directory> [--pm <manager>] [--no-install] [--force]')
      return 1
    }
  ...
  if (command === 'init') {
    if (positionals.length > 0) {
      console.error('Usage: muse init [--force] [--no-install]')
      return 1
    }
    return scaffold(process.cwd(), 'init')
  }
  console.error(`Unknown Muse command: ${command}`)
  printHelp()
  return 1
}
...
try {
  ...
  console.error(`Muse ${command ?? 'command'} failed: ${error instanceof Error ? error.message : String(error)}`)
  process.exitCode = 1
}
