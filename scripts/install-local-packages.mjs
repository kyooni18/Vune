...
for (const file of readdirSync(packDir)) {
  ...
  const match = file.match(/^(vune-ui(?:-(?:animation|core|compiler|execution|legacy-react|react|vue|web|vite))?)-(.+)\.tgz$/u)
  if (!match) continue
  const short = match[1]
...
for (const required of ['vune-ui', '@vune-ui/animation', '@vune-ui/core', '@vune-ui/compiler', '@vune-ui/execution', '@vune-ui/vite', `@vune-ui/${rendererArg}`]) {
