...
export function Label(
  title: VNodeChild | Value<string | number>,
  icon: VNodeChild,
...
export function Link(
  ...
  return styled(
    h(
      'a',
      mergeProps(props, { href: String(toValue(href)) }),
      content(label),
    ),
  )
}

export type ProgressViewOptions = NativeProps & {
  max?: number
  label?: VNodeChild | Value<string | number>
}
...
export type SliderOptions = NativeProps & {
  min?: number
  max?: number
  step?: number
}
